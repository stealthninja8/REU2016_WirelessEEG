...
public class DataMarking {
    ...
    public static void main(String args[]) throws Exception {
        ...
        for (int k = 0; k < maxes.size(); k++) {
            System.out.println(maxes.get(k) + "\t" + mins.get(k));
        }
        j=0;
        ...
        int minPos = j;
        double closest = abs(value2.get(j));
        while(j<i-10){
            double midline = (maxes.get(j-10)+mins.get(j-10))/2;
            if(value2.get(j) <= midline+0.001 && value2.get(j) >= midline-0.001){
                if(value2.get(j-10)> value2.get(j+10) && abs(closest - midline) > abs(value2.get(j)-midline)){
                    mark[j] = 1;
                    
                    mark[minPos]=0;
                    minPos = j;
                    closest = abs(value2.get(j) - midline);
